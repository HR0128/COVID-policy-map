...
reader=pd.read_csv("no_elective_integrated.csv")

Startweek=sorted(list(set(reader["Startweek"])))

#Convert policy to index
policyname=reader["PolicyName"].astype(str)
no_elective_process_dict={ 'No Elective Procedures Start':1, 'No Elective Procedures End':-1}
...
#reader.to_csv("no elective_policyasindex.csv")
...
#Diffform.to_csv("no elective_diffform.csv")
...
Sumform.to_csv("no elective_sumform.csv")
