...
reader=pd.read_csv("mask_employees_integrated.csv")


#Convert policy to index
policyname=reader["PolicyName"].astype(str)
masks_em_dict={'Employee Mandatory':2, 'Employee Recommended':1, 'Employee None': -5}

policyindex=[]
for i in policyname:
        policyindex.append(masks_em_dict[i])
...
#reader.to_csv("mask_employees_policyasindex.csv")
...
#Diffform.to_csv("mask_employees_diffform.csv")
...
Sumform.to_csv("mask_employees_sumform.csv")
