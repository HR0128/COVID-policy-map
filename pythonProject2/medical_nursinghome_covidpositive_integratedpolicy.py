...
reader=pd.read_csv("medical_nursinghome_covidpositive.csv", converters={"FIPSCounty": '{:0>5}'.format})
...
reader=reader.sort_values(by=["StartDate"])

reader=reader.drop_duplicates(subset=["FIPSCounty", "Startweek"], keep='last')

reader.to_csv("medical_nursinghome_covidpositive_integrated.csv")